...
func runAdd(ctx context.Context, args []string, globalOutput OutputFormat, deps Deps) error {
	fs, f := newAddFlagSet(deps.Stderr, globalOutput)
	...
	resp, err := svc.Add(ctx, alidns.AddInput{
		DomainName: f.domain,
		Name:       f.name,
		Type:       f.rType,
		Value:      f.value,
